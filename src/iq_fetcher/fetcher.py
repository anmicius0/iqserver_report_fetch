...
from .utils import logger, resolve_path, ErrorHandler
...
class RawReportFetcher:
    ...
    def _extract_report_id(self, info: ReportInfo) -> Optional[str]:
        ...
        if info.report_data_url:
            try:
                report_id = info.report_data_url.split("/reports/")[1].split("/")[0]
        ...
        fallback_id = info.scan_id or info.report_id
    ...
    def _fetch_app_report(
        ...
        logger.debug(f"[{idx}/{total}] 🚦 Processing {app.name} ({app.public_id}) ...")

        info = self.iq.get_latest_report_info(app.id)
        if not info:
            logger.warning(
                f"[{idx}/{total}] ❗ No reports found for {app.name} ({app.public_id})"
            )
            return None
        ...
        if not report_id:
            logger.warning(
                f"[{idx}/{total}] ❗ No report ID for {app.name} ({app.public_id})"
            )
            return None

        data = self.iq.get_policy_violations(app.public_id, report_id)
        if not data:
            logger.warning(
                f"[{idx}/{total}] ❗ No report data for {app.name} ({app.public_id})"
            )
            return None

        # Save JSON to disk
        json_filename = f"{app.public_id}_{report_id}.json"
    ...
    def get_applications(self) -> List[Application]:
        ...
        if apps:
            ...
            for i, app in enumerate(apps[:5], 1):
                logger.info(f"   {i}. {app.name} ({app.public_id})")
            if len(apps) > 5:
                ...
                for future in concurrent.futures.as_completed(future_to_app):
                    ...
                    try:
                        ...
                        if json_path:
                            ...
                            success_count += 1
                            logger.debug(
                                f"Report fetched and saved for {app.name} ({app.public_id}) at {json_path}"
                            )
                    except Exception as e:
                        logger.error(
                            f"❌ Error fetching report for {app.name} ({app.public_id}): {e}"
                        )
                    finally:
                        pbar.update(1)
